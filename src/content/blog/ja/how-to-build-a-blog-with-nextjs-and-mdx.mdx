---
title: Next.js と MDX でブログを構築する方法
date: '2022-08-22T00:00:00Z'
modifiedTime: '2025-09-16T15:11:03Z'
summary: Next.js でブログを作るのはとても簡単です！
---
...
- [next](https://github.com/vercel/next.js) Next.js フレームワーク
- [next-mdx-remote](https://github.com/hashicorp/next-mdx-remote) MDX コンテンツの処理と読み込み
...
- `pages/blog/[slug].jsx` - 記事ページ、[動的ルーティング](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes)を使用
...
まず `const root{:js}` はルートディレクトリで、`process.cwd(){:js}` メソッドは Node.js プロセスの現在の作業ディレクトリを返します。

```js title='lib/mdx.js'
const root = process.cwd()
```

記事ファイルが保存されるパスのための変数 `POSTS_PATH{:.entity.name.constant}` も定義します。

```js title='lib/mdx.js'
import path from 'path'

const POSTS_PATH = path.join(root, 'data', 'blog') // [!code highlight]
// 出力例：A:/nextjs-mdx-blog/data/blog
```
...
// 出力例：['markdown.mdx', 'nextjs.mdx', 'react.mdx']
```
...
export const getPostBySlug = async (slug) => {
  ...
  // 出力例：A:/nextjs-mdx-blog/data/blog/slug.mdx
  ...
  // コンテンツを serialize (next-mdx-remote) で処理する

  const frontMatter = {
    ...data,
    slug,
  }
  // 後で使用するためにスラッグをフロントマターにも追加する
...
- [動的ルーティング](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes)
- [mdx](https://mdxjs.com/)
